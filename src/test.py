...
# Function to create a card using dmc.Paper with a background image
...
app = dash.Dash(external_stylesheets=[dmc.styles.CAROUSEL])

# App layout wrapped in dmc.MantineProvider to enable theme context
app.layout = (
    dmc.MantineProvider(
        children=[
            dmc.Center(
                children=[
                    dmc.Carousel(
                        children=[
                            dmc.CarouselSlide(make_card(d["image"], d["title"], d["category"]))
                            for d in data
                        ],
                        id="carousel-cards",
                        align="start",
